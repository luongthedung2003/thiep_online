...
<script>
// Global Cart Functionality Engine
(function() {
    const CART_KEY = 'wedding_portal_cart_data';
    ...
    window.proceedToCheckout = function() {
        let cart = getCartData();
        if (cart.length === 0) {
            alert('Giỏ hàng của bạn đang trống. Vui lòng chọn mẫu thiệp trước khi thanh toán!');
            return;
        }
        if (typeof openSocialModal === 'function') {
            openSocialModal('Thanh toán giỏ hàng');
        } else {
            window.location.href = '/mau-thiep';
        }
    };
    ...
    function renderCart() {
        ...
        const drawer = document.getElementById('offcanvasRight');
        if (!drawer) return;

        const cartList = drawer.querySelector('ul.list-none');
        if (!cartList) return;
        ...
        if (cart.length === 0) {
            cartList.innerHTML = `
                <li class="py-8 text-center text-gray-500">
                    ...
                    <p class="font-medium text-gray-700">Giỏ hàng đang trống</p>
                    <p class="text-xs text-gray-400 mt-1">Hãy thêm các mẫu thiệp yêu thích vào giỏ hàng nhé!</p>
                </li>
            `;
            return;
        }
            ...
            return `
                <li class="py-3 border-t">
                    <div class="flex items-center">
                        <div class="w-1/2 md:w-1/2 lg:w-3/5">
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    ...
                                    <span><small class="text-gray-500">Mẫu Thiệp Cưới</small></span>
                                    <div class="mt-2 small leading-none">
                                        <a href="javascript:void(0)" onclick="removeFromCart('${item.id}')" class="text-rose-500 flex items-center">
                                            <span class="mr-1 align-text-bottom">
                        ...
                        <div class="w-1/3 md:w-1/4 lg:w-1/5">
                            <div class="input-group input-spinner rounded-lg flex justify-between items-center">
                                <input type="button" value="-" onclick="changeCartQty('${item.id}', -1)" class="button-minus w-8 py-1 border-r cursor-pointer border-gray-300" />
                                <input type="number" step="1" max="10" value="${item.quantity}" readonly class="quantity-field w-9 px-2 text-center h-7 border-0 bg-transparent" />
                                <input type="button" value="+" onclick="changeCartQty('${item.id}', 1)" class="button-plus w-8 py-1 border-l cursor-pointer border-gray-300" />
                            </div>
                        </div>
                        <div class="w-1/5 text-center md:w-1/5">
                            <span class="font-bold text-gray-800">${displayPrice}</span>
                        </div>
                    </div>
                </li>
            `;
